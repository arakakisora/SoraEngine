		...
		while (std::getline(lineStream, cell, ','))
		{
			if (y < kNumBlockVertical && x < kNumBlockHorizontal)
			{
				...
				if (id == MapChipType::Portal)
				{
					...
					if (std::getline(cellParser, pairText, ':') && std::getline(cellParser, dirText, ':'))
					{
...
std::vector<Vector3> MapChipField::GetEnemyPositions()
{
	...
	for (uint32_t y = 0; y < kNumBlockVertical; ++y)
	{
		for (uint32_t x = 0; x < kNumBlockHorizontal; ++x)
		{
			...
			if (info->spawn == "enemy")
			{
				enemyPositions.push_back(GetMapChipPositionByIndex(x, y));
				enemyNumber_.push_back(info->enemyNumber); // enemyNumber をそのまま保存
			}
		}
	}
...
Vector3 MapChipField::GetGoalPosition()
{
	...
	for (uint32_t y = 0; y < kNumBlockVertical; ++y)
	{
		for (uint32_t x = 0; x < kNumBlockHorizontal; ++x)
		{
			...
			if (info->spawn == "goal")
			{
				pos = GetMapChipPositionByIndex(x, y);
				// 複数ある場合は最後のものが採用される
			}
		}
	}
...
std::vector<Vector3> MapChipField::GetPositionBySpawn(const std::string& spawnTag)
{
	...
	for (uint32_t y = 0; y < kNumBlockVertical; ++y)
	{
		for (uint32_t x = 0; x < kNumBlockHorizontal; ++x)
		{
			...
			{
				result.push_back(GetMapChipPositionByIndex(x, y));
			}
		}
	}
...
bool MapChipField::TryGetPairPortal(const PortalInfo& in, PortalInfo& out) const
{
	for (const PortalInfo& portal : portals_)
	{
		if (portal.pairId == in.pairId && !(portal.x == in.x && portal.y == in.y))
		{
			out = portal;
			return true;
		}
