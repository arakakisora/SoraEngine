...
void StageEditor::Run()
{
	...
	{
		strokeVisited_.resize(stageData_.GetHeight(), std::vector<bool>(stageData_.GetWidth(), false));
	}
...
void StageEditor::RenderUI()
{
	...
	if (fs::exists("Resources/Mapdata") && fs::is_directory("Resources/Mapdata"))
	{
		...
		if (newList != availableStages)
		{
			...
			{
				if (selectedStageIndex >= availableStages.size())
					selectedStageIndex = 0;
				strncpy_s(fileNameBuffer, sizeof(fileNameBuffer), availableStages[selectedStageIndex].c_str(),
						  _TRUNCATE);
	...
	if (hoverX >= 0 && hoverY >= 0)
	{
		...
		const MapChipInfo* hoverInfo = MapChipDatabase::GetInstance()->GetById(stageData_.GetType(hoverX, hoverY));
	...
	while (std::getline(file, line))
	{
		...
		while (std::getline(lineStream, cellText, ','))
		{
			...
			if (type == MapChipType::Portal)
			{
				...
				if (std::getline(cellParser, linkText, ':') && std::getline(cellParser, dirText, ':'))
				{
...
void StageEditor::ApplyCellWithUndo(int x, int y, int newType)
{
	if (x < 0 || y < 0)
		return;
	if (x >= static_cast<int>(stageData_.GetWidth()))
		return;
	if (y >= static_cast<int>(stageData_.GetHeight()))
		return;
...
void StageEditor::Undo()
{
	...
	{
		stageData_.SetType(static_cast<uint32_t>(it->x), static_cast<uint32_t>(it->y),
...
void StageEditor::Redo()
{
	...
	for (auto& e : s)
	{
		stageData_.SetType(static_cast<uint32_t>(e.x), static_cast<uint32_t>(e.y), static_cast<MapChipType>(e.after));
	}
