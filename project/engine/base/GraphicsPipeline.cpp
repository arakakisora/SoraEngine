...
void GraphicsPipeline::CreateCopyImage()
{
	...
	// シェーダーを読み込み
	IDxcBlob* vertexshaderBlob = dxCommon_->CompileShader(L"Resources/Shaders/Vignette.VS.hlsl", L"vs_6_0");
	assert(vertexshaderBlob != nullptr);

	IDxcBlob* pixelShaderBlob = dxCommon_->CompileShader(L"Resources/Shaders/Vignette.PS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
