...
void GraphicsPipeline::CreateCompute()
{
	RootSignatureComputeCreate();

	IDxcBlob* computeShaderBlob = dxCommon_->CompileShader(
		L"Resources/Shaders/ComputeShader.hlsl", L"cs_6_0");
	...
	HRESULT hr = dxCommon_->GetDevice()->CreateComputePipelineState(
		&computePipelineStateDesc, IID_PPV_ARGS(&graphicsPipelineStateCompute));
	assert(SUCCEEDED(hr));
}

void GraphicsPipeline::RootSignatureComputeCreate()
{
	// ComputeShader 用 RootSignature の作成
	D3D12_ROOT_SIGNATURE_DESC desc{};
	desc.Flags = D3D12_ROOT_SIGNATURE_FLAG_NONE;

	// ===== DescriptorRange 設定 =====

	// SRV3つ (t0～t2)：MatrixPalette, InputVertices, Influences
	D3D12_DESCRIPTOR_RANGE srvRanges{};
	srvRanges.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	srvRanges.NumDescriptors = 3;
	srvRanges.BaseShaderRegister = 0; // t0 から始まる
	srvRanges.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	// UAV1つ (u0)：OutputVertices
