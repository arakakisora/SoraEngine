...
D3D12_GPU_DESCRIPTOR_HANDLE GetGPUDesctiptorHandle(ID3D12DescriptorHeap* descriptorHeap, uint32_t descriptorSize, uint32_t index) {
...
#pragma region Resource
	const uint32_t kSubdbivision = 16;
	...
	*materialDataSprite->color = Vector4(0.0f, 1.0f, 1.0f, 1.0f);
	...
	//データを書き込む
	Matrix4x4* transformaitionMatrixDataSprite = nullptr;
	//書き込むためのアドレスを取得
...
#pragma region Texturを読む
	...
	ID3D12Resource* intermediateResouce = UploadTextureData(textureResource, mipImages, device, commandList);
...
#pragma region CommandList 
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			*transformaitionMatrixDataSprite = worldViewProjectionMatrixSprite;
